(function ( angular ) {
  ...
  angular.module( "mdxUtil" ).factory( 'mdxColor', function ( mdxMDPalettes ) {

    //noinspection JSUnusedGlobalSymbols
    return {
      palette : {
        getMD    : function getMDPalette( name ) {
          name = lookupName( name, mdxMDPalettes );
          return mdxMDPalettes[name];
        },
        simulate : function simulate( hex, ignoreMD ) {
        ...
        analyze  : function analyze() {
          ...
          angular.forEach( mdxMDPalettes, function ( palette, key ) {
            var hsl = {};
            var hIdx,sIdx,lIdx;
            for (hIdx = 0; hIdx < lo.length;) {
              hsl[key] = hsl[key] || [];
              hsl[key].push(lo[hIdx])
            }
  ...
  // Verify palette by actual name or check 500 value
  function lookupName( name, palettes ) {
    ...
    if ( !palettes[name] ) {
      angular.forEach( palettes, function ( value, key ) {
        var alt = RGB_to_HEX( HEX_to_RGB( name ) );
        var vname = RGB_to_HEX( HEX_to_RGB( value['500'] ) );
